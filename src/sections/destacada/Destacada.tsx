 ...
 *
 * La foto sangra hasta el borde derecho de la ventana, así que la sección NO
 * puede ir dentro del contenedor centrado de 1520: el texto se alinea a mano
 * con ese margen (`calc((100vw - 1520px) / 2)`) y la imagen ocupa el resto.
 */
export function Destacada({ noticia }: { noticia: Noticia }) {
  const margen = "desk:pl-[max(40px,calc((100vw-1520px)/2))]";
  const { fuerte, resto } = partirTitulo(noticia.titulo);

  return (
    <section className="w-full bg-panel">
      <div className="flex flex-col desk:flex-row desk:items-center">
        <div
          className={`flex flex-1 flex-col gap-[16px] px-[20px] py-[36px] desk:gap-[22px] desk:py-[44px] desk:pr-[56px] ${margen} ${
            noticia.imagen ? "" : "desk:pr-[max(40px,calc((100vw-1520px)/2))]"
          }`}
        >
          ...
          <div className="flex flex-wrap items-center gap-x-[28px] gap-y-[10px]">
            ...
            <Link
              ...
              className="flex items-center gap-[8px] font-texto text-[15px] font-bold text-rojo desk:text-[16px]"
            >
              LEER NOTA
              <IconoFlecha className="h-[16px] w-[16px] shrink-0" />
            </Link>
          </div>
        </div>
        ...
        {noticia.imagen && (
          <Link
            to={noticiaHref(noticia.slug)}
            // El borde izquierdo va en diagonal, como en el Figma.
            className="block h-[220px] w-full shrink-0 overflow-hidden desk:h-[505px] desk:w-[54.5%] desk:[clip-path:polygon(5%_0,100%_0,100%_100%,0_100%)]"
          >
            <img
              src={noticia.imagen}
              alt={noticia.titulo}
